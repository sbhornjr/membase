...
class TransactionManager():
    def __init__(self, db, pm, ns = None):
        self.db = db
    ...
    def commit(self):
        ...
        if self.transactions_active == 0:
            self.persistence_manager.add_commands(self._setup_commands_for_log(committed), self.namespace)
        else:
    ...
    def add_command(self, key, value=None):
        if self.transactions_active == 0 or not self.transactions_stack:
            print("transaction not started")
            return
        self.transactions_stack[-1].setdefault(key, []).append(self.db.get(key, self.namespace))
        if self.history_stack:
    ...
    def _undo(self, key, op_list):
        for value in op_list[::-1]:
            if value:
                self.db.set(key, value, self.namespace, False)
            else:
                self.db.delete(key, self.namespace, False)

    def _commit_history(self):
        committed_history = self.history_stack.pop()
        if self.transactions_active == 0:
            self.db.extend_history(committed_history, self.namespace)
        else:
    ...
    def _setup_commands_for_log(self, commands):
        ...
        for command in commands:
            for op in commands[command]:
                ...
                if self.db.get(command, self.namespace):
                    log_commands.append(f"set {command} {self.db.get(command, self.namespace)}")
